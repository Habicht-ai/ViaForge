...
import com.viaversion.viaversion.protocols.v26_2to26_3.packet.*;
...
final class FlattenedPipelineSmokeTest {
    ...
    private final boolean bee, nether, netherPatch, caves, cavesPatch, cliffs, wild, wildPatch, wild3, wild4, trails, config, trials, components, tricky, bundles, winter, spring, summer, summerPatch, copper, mounts, year26, sulfur, wilderness;
    ...
    private void transportClock()throws Exception {
        if(caves)for(int value:new int[]{Integer.MIN_VALUE,-1,0,32768,Integer.MAX_VALUE}) {
            drain(client);drain(server);
            ByteBuf ping=packetName("PING");ping.writeInt(value);receive(ping);client.runPendingTasks();
            require(client.readOutbound()==null&&server.readInbound()==null,"Ping must not acknowledge before the game thread");
            ByteBuf event=take(0x3f);
            require(OrderedPing.CHANNEL.equals(Types.STRING.read(event))&&event.readInt()==value&&!event.isReadable(),"Full signed ping ID survives native event");event.release();
            ByteBuf reply=BlockPipelineSmokeTest.packet(0x17);Types.STRING.write(reply,OrderedPing.CHANNEL);reply.writeInt(value);send(reply);
            ByteBuf pong=server.readInbound();require(pong!=null,"Game-thread reply reaches server");
            require(Types.VAR_INT.readPrimitive(pong)==outgoingName("PONG")&&pong.readInt()==value&&!pong.isReadable(),"Exactly one original PONG with exact 32-bit ID");pong.release();
            require(server.readInbound()==null,"No duplicate proxy pong");
        }
        if(bundles) {
            drain(client);drain(server);
            new com.viaversion.viabackwards.protocol.v1_21_2to1_21.task.PlayerPacketsTickTask().run(user,user.get(com.viaversion.viabackwards.protocol.v1_21_2to1_21.storage.PlayerStorage.class));
            client.runPendingTasks();require(client.readOutbound()==null,"Proxy clock cannot invent client ticks");
            for(int flags:new int[]{0,16,0,65,32}) {
                ByteBuf input=BlockPipelineSmokeTest.packet(0x17);Types.STRING.write(input,ClientTickPackets.CHANNEL);input.writeByte(0).writeByte(flags);send(input);
                ByteBuf original=server.readInbound();require(original!=null&&Types.VAR_INT.readPrimitive(original)==outgoingName("PLAYER_INPUT")&&original.readByte()==flags&&!original.isReadable(),"Actual native input bits reach modern server");original.release();
            }
            ByteBuf tick=BlockPipelineSmokeTest.packet(0x17);Types.STRING.write(tick,ClientTickPackets.CHANNEL);tick.writeByte(1);send(tick);
            ByteBuf original=server.readInbound();require(original!=null&&Types.VAR_INT.readPrimitive(original)==outgoingName("CLIENT_TICK_END")&&!original.isReadable(),"One tick boundary after actual movement");original.release();
            require(server.readInbound()==null,"No duplicate tick or input");
            for(int i=0;i<3;i++) {
                ByteBuf move=BlockPipelineSmokeTest.packet(0x04);move.writeDouble(2+i).writeDouble(64).writeDouble(3).writeBoolean(true);send(move);
                original=server.readInbound();require(original!=null&&Types.VAR_INT.readPrimitive(original)==outgoingName("MOVE_PLAYER_POS"),"Movement must not synthesize an extra tick boundary");original.release();
                require(server.readInbound()==null,"Exactly one movement packet before native tick end");
            }
        }
    }
    ...
    private int modernBlockBits() {
        MappingData mapping=wilderness?Protocol26_2To26_3.MAPPINGS:sulfur?Protocol26_1To26_2.MAPPINGS:year26?Protocol1_21_11To26_1.MAPPINGS:mounts?Protocol1_21_9To1_21_11.MAPPINGS:copper?Protocol1_21_7To1_21_9.MAPPINGS:summerPatch?Protocol1_21_6To1_21_7.MAPPINGS:summer?Protocol1_21_5To1_21_6.MAPPINGS:Protocol1_21_4To1_21_5.MAPPINGS;
    ...
    private void joinConfigured()throws Exception {
        ...
        if(components)Types.VAR_INT.writePrimitive(packet,0);else Types.STRING.write(packet,"minecraft:overworld");Types.STRING.write(packet,"minecraft:overworld");packet.writeLong(12345L);if(wilderness){Types.VAR_INT.writePrimitive(packet,1);Types.OPTIONAL_VAR_INT.write(packet,null);}else packet.writeByte(1).writeByte(-1);packet.writeBoolean(false).writeBoolean(false);
    ...
    private int blocks(WorldClient world)throws Exception {
        ...
        if(village) {
            ...
            require(section.getExtSkylightValue(15,15,15)==10&&section.getExtBlocklightValue(15,15,15)==5,"Original separate sky/block light survived restoration: sky="+section.getExtSkylightValue(15,15,15)+", block="+section.getExtBlocklightValue(15,15,15));
        }
    ...
    private void drops(WorldClient world)throws Exception {
        ...
        DropItemSmokeTest.verify(world,target.serverProtocol()>=401,shield,ServerEntitySmokeTest.stack(201,0),new DropItemSmokeTest.Wire(){
            public void action(net.minecraft.network.play.client.C07PacketPlayerDigging packet)throws Exception {
                ...
                try{require(Types.VAR_INT.readPrimitive(data)==packet.getStatus().ordinal()+(wilderness&&packet.getStatus().ordinal()>=1?1:0),"Target drop action through every Via layer");
    ...
    private void eggs()throws Exception {
        for(com.google.gson.JsonElement entry:ItemVariants.EGGS) {
            ...
            if(sulfur) {
                ...
                int entityId=wilderness?com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_3.valueOf(modern.toUpperCase(java.util.Locale.ROOT)).getId():com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_2.valueOf(modern.toUpperCase(java.util.Locale.ROOT)).getId();
                ...
                if(species.equals("shulker")) {
                    ...
                    typed.dataContainer().set(StructuredDataKey.ENTITY_DATA1_21_9,new com.viaversion.viaversion.api.minecraft.item.data.EntityData(wilderness?com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_3.PIG.getId():com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_2.PIG.getId(),entity));
                    shown=slot(typed,36);
    ...
    private void flight(WorldClient world)throws Exception {
        ElytraFlightSmokeTest.verify(world,new ElytraFlightSmokeTest.Wire(){
            public void use(net.minecraft.network.play.client.C17PacketCustomPayload packet,boolean swing,int hand)throws Exception {
                require(!wilderness||!swing,"26.3 successful use must not send PUNCH");
                ...
                require(server.readInbound()==null,"No duplicate use/attack on target wire");
            }
            public void start(net.minecraft.network.play.client.C17PacketCustomPayload packet)throws Exception {
                ByteBuf source=BlockPipelineSmokeTest.packet(0x17);packet.writePacketData(new PacketBuffer(source));send(source);
                ByteBuf result=BlockPipelineSmokeTest.take(server,outgoing(ServerboundPackets1_13.PLAYER_COMMAND));
                try{require(Types.VAR_INT.readPrimitive(result)==1&&Types.VAR_INT.readPrimitive(result)==(summer?6:8)&&Types.VAR_INT.readPrimitive(result)==0&&!result.isReadable(),"Exact target START_FALL_FLYING action");}finally{result.release();}
            }
            public void flag(int entityId,boolean flying,net.minecraft.client.network.NetHandlerPlayClient handler)throws Exception {
                ByteBuf source=packet(ClientboundPackets1_13.SET_ENTITY_DATA);Types.VAR_INT.writePrimitive(source,entityId);source.writeByte(0).writeByte(0).writeByte(ElytraFlightSmokeTest.flags(entityId,flying)).writeByte(255);
                ServerEntitySmokeTest.send(client,server,handler,source);
            }
            public void rocket(int entityId,int targetId,net.minecraft.client.network.NetHandlerPlayClient handler)throws Exception {
                if(targetId>=0) {
                    ByteBuf spawn=packet(ClientboundPackets1_13.ADD_ENTITY);Types.VAR_INT.writePrimitive(spawn,entityId);Types.UUID.write(spawn,new UUID(0,entityId));
                    objectType(spawn,76);spawnCoordinates(spawn,5,200,5);spawn.writeByte(0).writeByte(0);objectData(spawn,0);spawnVelocity(spawn);
                    ServerEntitySmokeTest.send(client,server,handler,spawn);
                }
                ByteBuf data=packet(ClientboundPackets1_13.SET_ENTITY_DATA);Types.VAR_INT.writePrimitive(data,entityId);
                data.writeByte(caves?9:village?8:7);Types.VAR_INT.writePrimitive(data,!village?1:copper?19:components?20:wild4?19:wild3?18:17);
                if(village)Types.OPTIONAL_VAR_INT.write(data,targetId<0?null:targetId);else Types.VAR_INT.writePrimitive(data,Math.max(0,targetId));
                data.writeByte(255);ServerEntitySmokeTest.send(client,server,handler,data);
            }
        });
    }
    private void events()throws Exception {
        ...
        if(mounts) {
            ...
            arm.writeByte(15);Types.VAR_INT.writePrimitive(arm,wilderness?VersionedTypes.V26_3.entityDataTypes.humanoidArmType.typeId():year26?VersionedTypes.V26_1.entityDataTypes.humanoidArmType.typeId():VersionedTypes.V1_21_11.entityDataTypes.humanoidArmType.typeId());Types.VAR_INT.writePrimitive(arm,0);arm.writeByte(255);receive(arm);
    ...
    private void hands()throws Exception {
        ...
        ByteBuf attackData=BlockPipelineSmokeTest.take(server,wilderness?ServerboundPackets26_3.ATTACK.getId():year26?ServerboundPackets26_1.ATTACK.getId():outgoing(ServerboundPackets1_13.INTERACT));
    ...
    private void lifecycle()throws Exception {
        ...
        if(village) {
            ...
            require(((com.viaversion.viabackwards.protocol.v1_14to1_13_2.storage.ProtocolStorables1_14)user.storables(com.viaversion.viabackwards.protocol.v1_14to1_13_2.Protocol1_14To1_13_2.class)).chunkLightStorage().getStoredLight(0,0)!=null,"Reload populated the Via light cache before dimension change");
        }
    ...
    private void objectType(ByteBuf packet,int old){
        if(bundles&&old==1)Types.VAR_INT.writePrimitive(packet,wilderness?com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_3.DARK_OAK_BOAT.getId():sulfur?com.viaversion.viaversion.api.minecraft.entities.EntityTypes26_2.DARK_OAK_BOAT.getId():mounts?com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_21_11.DARK_OAK_BOAT.getId():copper?com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_21_9.DARK_OAK_BOAT.getId():summer?com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_21_6.DARK_OAK_BOAT.getId():spring?com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_21_5.DARK_OAK_BOAT.getId():com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_21_2.DARK_OAK_BOAT.getId());else if(village)Types.VAR_INT.writePrimitive(packet,wireEntity(com.viaversion.viaversion.api.minecraft.entities.EntityTypes1_13.ObjectType.findById(old,0).getType().getId()));else packet.writeByte(old);
    }
    private int outgoing(ServerboundPackets1_13 type){return outgoingName(type.name());}
    private int outgoingName(String name){return wilderness?ServerboundPackets26_3.valueOf(name.equals("SWING")?"PUNCH":name).getId():year26?ServerboundPackets26_1.valueOf(name).getId():summer?ServerboundPackets1_21_6.valueOf(name).getId():spring?ServerboundPackets1_21_5.valueOf(name).getId():winter?ServerboundPackets1_21_4.valueOf(name).getId():bundles?ServerboundPackets1_21_2.valueOf(name).getId():components?ServerboundPackets1_20_5.valueOf(name).getId():trials?com.viaversion.viaversion.protocols.v1_20_2to1_20_3.packet.ServerboundPackets1_20_3.valueOf(name).getId():config?ServerboundPackets1_20_2.valueOf(name).getId():wild4?ServerboundPackets1_19_4.valueOf(name).getId():wild3?ServerboundPackets1_19_3.valueOf(name).getId():wildPatch?ServerboundPackets1_19_1.valueOf(name).getId():wild?ServerboundPackets1_19.valueOf(name).getId():caves?ServerboundPackets1_17.valueOf(name).getId():netherPatch?ServerboundPackets1_16_2.valueOf(name).getId():nether?ServerboundPackets1_16.valueOf(name).getId():village?ServerboundPackets1_14.valueOf(name).getId():ServerboundPackets1_13.valueOf(name).getId();}
    private ByteBuf packet(ClientboundPackets1_13 type){return packetName(type.name());}
    private ByteBuf packetName(String name){
        if(nether&&name.equals("SET_EQUIPPED_ITEM"))name="SET_EQUIPMENT";if(netherPatch&&name.equals("CHUNK_BLOCKS_UPDATE"))name="SECTION_BLOCKS_UPDATE";
    ...
    private ByteBuf lightHeader()throws Exception {
        ...
        for(int value:new int[]{32,32,0,0}){if(wilderness)Types.BIT_SET.write(light,BitSet.valueOf(new long[]{(long)value<<4}));else if(caves)Types.LONG_ARRAY_PRIMITIVE.write(light,new long[]{(long)value<<4});else Types.VAR_INT.writePrimitive(light,value);}
        return light;
    }
