...
final class FlattenedPipelineSmokeTest {
    ...
    static String verify(CompatibilityProfile target,WorldClient world,Path report)throws Exception {
        ...
        try {
            test.join();int count=test.blocks(world);test.heightWindow(world);test.items();test.eggs();test.drops(world);test.events();test.boatInput(world);test.flight(world);test.hands();test.editors();test.resources(report);MobRenderSmokeTest.modern(target,world,report);test.lifecycle();
