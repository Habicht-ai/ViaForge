...
public final class MobSoundAssets {
    public static Map<String,byte[]> load(Path cache, String version) throws IOException {
        ...
        for (Map.Entry<String,JsonElement> event : definitions.entrySet()) if (event.getKey().startsWith("entity.") || event.getKey().startsWith("ambient.underwater.") || event.getKey().startsWith("block.")&&event.getKey().endsWith(".place") || event.getKey().equals("enchant.thorns.hit") || event.getKey().equals("item.elytra.flying")) events.add(event.getKey());
