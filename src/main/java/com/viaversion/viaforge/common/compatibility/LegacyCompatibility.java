...
final class LegacyCompatibility {
    static CompatibilityRegistry create() {
        ...
        VersionRules v262=v261.derive().rule(COLLISION_PORTION_BOUNCE,true).build();
        profiles.add(new CompatibilityProfile(776,v262,new ResourceProfile("26.2",com.viaversion.viaforge.blocks.resources.Year26ResourceConverter::convert),new FlattenedProtocolAdapter.Factory(776)));
        VersionRules v263=v262.derive().rule(SUPPRESS_GRAVITY_EQUAL_BOUNCE,true).rule(SERVER_OWNS_USE_SWING,true).build();
