...
final class LegacyCompatibility {
    static CompatibilityRegistry create() {
        ...
        VersionRules v115=v114.derive().rule(SHIFT_SWIM_INPUT,false).rule(ROTATION_ONLY_ENTITY_UPDATE,true).rule(ROCKET_USE_SWING,true).rule(PREDICT_ELYTRA_START,true).build();
        ...
        VersionRules v116=v115.derive().rule(DELAYED_WATER_EYES,true).rule(MINIMUM_FLUID_CURRENT,true).build();
        VersionRules v1162=v116.derive().rule(KEEP_ZERO_ENTITY_DELTA,true).build();
        ...
        VersionRules v117=v1162.derive().rule(SWIM_FEET_IN_WATER,true).rule(PRECISE_ENTITY_PUSH,true).build();
        ...
        VersionRules v1182=v118.derive().rule(PRECISE_MOVEMENT_PACKETS,true).rule(FIREWORK_HAND_TRAIL,true).build();
        ...
        VersionRules v120=v119.derive().rule(ROUND_ENTITY_DELTA,true).rule(SMALL_VEX_MODEL,true).build();
        VersionRules v1194=v120.derive().rule(CURRENT_AIR_SPRINT,true).build();
        for(int i=0;i<trails.length;i++)profiles.add(new CompatibilityProfile(trails[i],i==0?v120:v1194,
        ...
        VersionRules v1202=v1194.derive().rule(BOUNDED_INVENTORY_PREVIEW,true).build();
        ...
        profiles.add(new CompatibilityProfile(776,v261,new ResourceProfile("26.2",com.viaversion.viaforge.blocks.resources.Year26ResourceConverter::convert),new FlattenedProtocolAdapter.Factory(776)));
        VersionRules v263=v261.derive().rule(SERVER_OWNS_USE_SWING,true).build();
