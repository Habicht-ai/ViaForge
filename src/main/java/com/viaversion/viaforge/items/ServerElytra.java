...
public final class ServerElytra extends ItemArmor {
    ...
    @Override public ModelBiped getArmorModel(EntityLivingBase entity, ItemStack stack, int slot) {
        float partial = ((com.viaversion.viaforge.mixin.impl.items.MinecraftItemTimer)net.minecraft.client.Minecraft.getMinecraft()).viaForge$itemTimer().renderPartialTicks;
        // Forge calls this once before the armor and enchantment-glint passes.
        ((Wings)wings).pose(entity, partial);
        return wings;
    }
    private static final class Wings extends ModelBiped {
        ...
        void pose(Entity entity, float partial) {
            float[] pose = ServerElytraVisuals.wings(entity, partial);
            float x = pose[0], y = pose[1], z = pose[2];
            float height = !ServerElytraFlight.flying(entity) && entity.isSneaking() ? 3 : 0;
