...
public abstract class MixinElytraMovement {
    ...
    private void glide(float strafe, float forward, CallbackInfo ci) { if (ServerElytraFlight.move((EntityPlayer)(Object)this)) ci.cancel(); }
    @Inject(method="getEyeHeight", at=@At("HEAD"), cancellable=true)
    private void flightCamera(CallbackInfoReturnable<Float> ci) {
        EntityPlayer player=(EntityPlayer)(Object)this;
        if (ServerElytraFlight.flying(player)||ServerElytraFlight.compact(player)) ci.setReturnValue(.4F);
