    ...
    with socket.create_connection(("127.0.0.1", row["port"]), timeout=20) as sock:
        ...
        for _ in range(2000):
            ...
            if phase == "configuration":
                ...
                continue
            if modern and packet == report["play"]["clientbound"]["minecraft:login"]["protocol_id"]:
                # Original 26.2 ClientboundLoginPacket: entity ID, hardcore, dimension keys,
                # max players, chunk radius, simulation distance (verified against its codec).
                source.read(5)
                for _ in range(read_vi(source)):
                    source.read(read_vi(source))
                read_vi(source)
                view_distance = read_vi(source)
                continue
            ...
            if packet == position_packet:
                ...
                result = {"success": True, "protocol": row["protocol"], "spawn": position,
                          "real_offline_login": True, "view_distance": view_distance, "rendering_test": False, "time": time.time()}
